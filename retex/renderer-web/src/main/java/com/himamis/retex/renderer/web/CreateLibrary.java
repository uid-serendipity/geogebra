...
public class CreateLibrary /* implements EntryPoint */{
	...
	public static void exportLibrary(JlmApi api) {
		if (Js.isTruthy(JlmReference.jlmlib)) {
			FactoryProvider.debugS("JLM already installed");
			return;
		}

		if ("function".equals(Js.typeof(JlmReference.jlmOnInit))) {
			JlmReference.jlmOnInit.call(DomGlobal.window, api);
		} else {
			JlmReference.jlmlib = api;
		}
	}
